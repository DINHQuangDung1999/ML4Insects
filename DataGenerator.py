...
def train_validation_test(X,y,r = 0.2, model_type = 'mlp'):
    '''
        Input: 
    ...
    '''
    df, lab = X['data'], X['label']
    df_test,lab_test = y['data'], y['label']
...
def get_loader(X,y,r=0.2,batch_size = 32, model_type = 'mlp'):
    '''
        Input: 
            X: dictionary with keys (data,label) - Data for training and validation
            y: dictionary with keys (data,label) - Data for testing
    ...
    '''    
    x_train, x_validation, x_test, y_train, y_validation, y_test = train_validation_test(X,y,r,model_type)
